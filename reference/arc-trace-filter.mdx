---
title: trace_filter | Arc
openapi: /openapi/arc_node_api/debug_and_trace/trace_filter.json POST /
description: "Arc API method that returns execution traces matching specific filter criteria. Available on Arc via Chainstack JSON-RPC nodes."
---
...
The following example finds all calls to the USDC token contract:
...
const traceFilter = async () => {
    ...
    console.log(`Found ${traces.length} traces to USDC`);
...
print(f"Found {len(traces)} traces to USDC")
