...
description: "The Solana getTransaction method returns detailed information about a specific transaction using its signature. Solana via Chainstack."
---
...
<Warning>
**Transaction version 1:** Solana mainnet now produces [version 1 transactions](https://solana.com/upgrades/larger-transaction-sizes). Pass `maxSupportedTransactionVersion: 1` to retrieve them—a lower value responds with error `-32015`.
</Warning>
...
* `config` (optional) — configuration object:
  ...
  + `maxSupportedTransactionVersion` — the highest transaction version your client can parse, as an integer in the `0`–`255` range. Set it to `1` to retrieve [version 1 transactions](https://solana.com/upgrades/larger-transaction-sizes), which Solana mainnet now produces. A lower value, or an omitted field, makes the call respond with error `-32015` when the transaction is newer than the version you declare.
