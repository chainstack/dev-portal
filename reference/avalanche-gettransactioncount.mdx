...
The function retrieves the `gasPrice` and `gasLimit` for the transaction using `web3.eth.getGasPrice` and `web3.eth.estimateGas`, respectively. The [eth\_gasPrice](/reference/avalanche_node_api/gas_data/eth_gasPrice) is the amount of gas that the transaction sender is willing to pay per unit of gas consumed by the transaction, while the [eth\_estimateGas](/reference/avalanche_node_api/gas_data/eth_estimateGas) is an estimate of the maximum amount of gas that the transaction will consume.
