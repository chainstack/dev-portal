...
Ethereum API method that traces the execution of [eth\_call](/reference/ethereum_node_api/execute_transactions/eth_call) within the context of a specific block's execution. This method uses the final state of the parent block as its base and allows developers to trace the execution of a particular call.
