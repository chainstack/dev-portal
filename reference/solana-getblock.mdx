...
<Warning>
**Transaction version 1:** Solana mainnet now produces [version 1 transactions](https://solana.com/upgrades/larger-transaction-sizes). Pass `maxSupportedTransactionVersion: 1` to retrieve recent blocks—a lower value responds with error `-32015`.
</Warning>
...
2. `config` (object, optional) — configuration object containing:
   ...
   * `maxSupportedTransactionVersion` (integer) — the highest transaction version your client can parse, in the `0`–`255` range. Set it to `1` to include [version 1 transactions](https://solana.com/upgrades/larger-transaction-sizes), which Solana mainnet now produces. A lower value, or an omitted field, makes the whole call respond with error `-32015` when the block holds a transaction newer than the version you declare.
