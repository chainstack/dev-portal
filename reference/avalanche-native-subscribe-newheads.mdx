...
* `object`— a block object with the following fields:
  ...
  * `transactions` — an array of transaction objects. See [eth\_getTransactionByHash](/reference/avalanche_node_api/transactions_info/eth_getTransactionByHash) for the exact shape.
