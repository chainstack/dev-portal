---
title: eth_newFilter | Arc
openapi: /openapi/arc_node_api/filter_handling/eth_newFilter.json POST /
description: "Arc API method that creates a new filter for logs matching specified criteria. The filter can be polled with eth_getFilterChanges."
---

Arc API method that creates a new filter for logs matching specified criteria. The filter can be polled with `eth_getFilterChanges`.

<Visibility for="humans">
<Check>
**Get your own node endpoint today**

[Start for free](https://console.chainstack.com/) and get your app to production levels immediately. No credit card required.

You can sign up with your GitHub, X, Google, or Microsoft account.
</Check>
</Visibility>

## Parameters

* `filterObject` — the filter object:
  * `fromBlock` — (optional) block number (hex) or tag to start from
  * `toBlock` — (optional) block number (hex) or tag to end at
  * `address` — (optional) contract address or array of addresses
  * `topics` — (optional) array of topic filters

## Response

* `result` — the filter ID

## `eth_newFilter` code examples

<CodeGroup>
```javascript ethers.js
const ethers = require('ethers');
const NODE_URL = "CHAINSTACK_NODE_URL";
const provider = new ethers.JsonRpcProvider(NODE_URL);

const USDC = "0x3600000000000000000000000000000000000000";

const createFilter = async () => {
    const filterId = await provider.send("eth_newFilter", [{
      address: USDC,
      fromBlock: "latest",
      toBlock: "latest"
    }]);
    console.log(`Filter ID: ${filterId}`);
  };

createFilter();
```

```python web3.py
from web3 import Web3

node_url = "CHAINSTACK_NODE_URL"
web3 = Web3(Web3.HTTPProvider(node_url))

USDC = "0x3600000000000000000000000000000000000000"

filter_id = web3.eth.filter({
    'address': USDC,
    'fromBlock': 'latest'
})
print(f"Filter ID: {filter_id.filter_id}")
```

```bash cURL
curl -X POST "CHAINSTACK_NODE_URL" \
  -H "Content-Type: application/json" \
  -d '{
    "jsonrpc": "2.0",
    "method": "eth_newFilter",
    "params": [{"address": "0x3600000000000000000000000000000000000000", "fromBlock": "latest"}],
    "id": 1
  }'
```
</CodeGroup>
