...
description: "Polygon API method that traces a specific transaction. trace_transaction JSON-RPC method available on the Polygon blockchain via Chainstack."
---
...
<Warning>
**Not available on Polygon.** Chainstack serves Polygon on the Bor client (archive uses Bor with PBSS), which does not implement the Erigon-only `trace_*` namespace. Use [`debug_traceTransaction`](/reference/polygon-tracetransaction) with the `flatCallTracer`, which returns an equivalent trace on Bor in the same Parity flat format as `trace_transaction`.
</Warning>
