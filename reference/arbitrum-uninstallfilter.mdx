...
* [eth\_newFilter](/reference/arbitrum_node_api/logs_and_events/eth_newFilter)
* [eth\_newBlockFilter](/reference/arbitrum_node_api/blocks_info/eth_newBlockFilter)
...
One use case for `eth_uninstallFilter` is to optimize resource usage in a DApp. When a DApp needs to monitor events on the blockchain, it can create a filter using [eth\_newFilter](/reference/arbitrum_node_api/logs_and_events/eth_newFilter) or [eth\_newBlockFilter](/reference/arbitrum_node_api/blocks_info/eth_newBlockFilter) to listen for specific events or blocks.
