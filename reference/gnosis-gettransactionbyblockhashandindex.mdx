...
description: "Gnosis Chain API method that retrieves information about a specific transaction based on the block hash and the transaction index within the block."
---
...
async function getSendersInBlock(blockHash) {
  ...
  for (let i = 0; i < transactions.length; i++) {
    const transaction = await provider.send("eth_getTransactionByBlockHashAndIndex", [blockHash, ethers.toBeHex(i)]);
