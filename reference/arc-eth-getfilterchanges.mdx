---
title: eth_getFilterChanges | Arc
openapi: /openapi/arc_node_api/filter_handling/eth_getFilterChanges.json POST /
description: "Arc API method that returns an array of logs or block/transaction hashes that occurred since the last poll. Arc via Chainstack."
---

Arc API method that returns an array of logs or block/transaction hashes that occurred since the last poll. Used with filters created by `eth_newFilter`, `eth_newBlockFilter`, or `eth_newPendingTransactionFilter`.
...
curl -X POST "CHAINSTACK_NODE_URL" \
  ...
  -d '{
    ...
    "params": [{"address": "0x3600000000000000000000000000000000000000"}],
    "id": 1
  }'
