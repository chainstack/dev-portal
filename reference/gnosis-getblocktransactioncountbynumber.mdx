...
Using `eth_getBlockTransactionCountByNumber` is a bit more straightforward compared to the [example shown using `eth_getBlockTransactionCountByHash`](/reference/gnosis_node_api/blocks_info/eth_getBlockTransactionCountByHash) for this same use case, you don't need to extract the block hash. Instead, you can query the transaction count directly.
