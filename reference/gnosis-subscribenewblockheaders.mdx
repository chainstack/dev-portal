...
Gnosis Chain web3.js subscription equivalent to [eth\_newBlockFilter](/api-reference/gnosis_node_api/blocks_info/eth_newBlockFilter). `subscribe("newBlockHeaders")` allows developers to subscribe to real-time updates about new block headers on the Gnosis Chain; the application will receive notifications whenever a new block is added to the blockchain. The notification will include information about the new block, such as its block number, hash, and timestamp.
...
* `object`— a block object with the following fields:
  ...
  * `transactions` — an array of transaction objects. See [eth\_getTransactionByHash](/api-reference/gnosis_node_api/transaction_info/eth_getTransactionByHash) for the exact shape.
