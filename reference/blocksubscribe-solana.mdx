...
description: "Subscribe to real-time Solana block updates with blockSubscribe. Receive WebSocket notifications whenever new blocks are confirmed on the network."
---
...
<Warning>
  **Transaction version 1:** Solana mainnet now produces [version 1 transactions](https://solana.com/upgrades/larger-transaction-sizes). Pass `maxSupportedTransactionVersion: 1` to receive blocks—at a lower value the subscription keeps delivering notifications, but every one of them carries `block: null` and `err: {"UnsupportedTransactionVersion": 1}` instead of block data.
</Warning>
...
* `maxSupportedTransactionVersion` - (optional) An integer in the `0`–`255` range declaring the highest transaction version your client can parse. Set it to `1` to receive [version 1 transactions](https://solana.com/upgrades/larger-transaction-sizes), which Solana mainnet now produces. At a lower value, or with the field omitted, each notification arrives with `block: null` and an `err` of `{"UnsupportedTransactionVersion": <version>}`. Pass the integer `1`, not the string `"1"`—a string fails the subscription request with error `-32602`. Subscriptions that set `transactionDetails` to `signatures` or `none` return no transaction bodies and are unaffected.
...
<CodeGroup>
  ...
  > {"jsonrpc":"2.0","id":"1","method":"blockSubscribe","params":["all",{"encoding":"json","maxSupportedTransactionVersion":1,"transactionDetails":"full","rewards":false}]}
  ```
  ...
  > {"jsonrpc":"2.0","id":"1","method":"blockSubscribe","params":[{"mentionsAccountOrProgram":"6EF8rrecthR5Dkzon8Nwu78hRvfCKubJ14M5uBEwF6P"},{"commitment":"confirmed","encoding":"base64","showRewards":false,"transactionDetails":"full","maxSupportedTransactionVersion":1}]}
  ```
</CodeGroup>
