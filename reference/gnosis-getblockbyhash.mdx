...
* `object` — block object, or `null` when no block was found.
  ...
  + `transactions` — the array of transaction objects. See [eth\_getTransactionByHash](/api-reference/gnosis_node_api/transaction_info/eth_getTransactionByHash) for the exact shape.
