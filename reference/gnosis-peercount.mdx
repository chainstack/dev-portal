...
description: "Gnosis Chain API method that returns the number of peers connected to the node. Available on Gnosis via Chainstack JSON-RPC nodes."
---
...
function checkPeerCount() {
  provider.send("net_peerCount").then((result) => {
    const peerCount = Number(result);
