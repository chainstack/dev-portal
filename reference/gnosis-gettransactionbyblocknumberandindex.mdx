...
description: "Gnosis Chain API method that retrieves information about a specific transaction based on the block number and the transaction index within the block."
---
...
async function getTypeInBlock() {
  ...
  for (let i = 0; i < transactions.length; i++) {
    const transaction = await provider.send("eth_getTransactionByBlockNumberAndIndex", [ethers.toBeHex(latestBlock), ethers.toBeHex(i)]);
