---
title: eth_getCode | Arc
openapi: /openapi/arc_node_api/accounts_info/eth_getCode.json POST /
description: "Arc API method that returns the bytecode at a given address. This is commonly used to verify if an address is a contract. On Arc."
---

Arc API method that returns the bytecode at a given address. This is commonly used to verify if an address is a contract.

<Visibility for="humans">
<Check>
**Get your own node endpoint today**

[Start for free](https://console.chainstack.com/) and get your app to production levels immediately. No credit card required.

You can sign up with your GitHub, X, Google, or Microsoft account.
</Check>
</Visibility>

## Parameters

* `address` — the address to get the code from
* `blockParameter` — the block number (hex) or tag (`latest`, `earliest`, `pending`)

## Response

* `result` — the bytecode at the given address, or `0x` if no code exists (EOA)

## `eth_getCode` code examples

<CodeGroup>
```javascript ethers.js
const ethers = require('ethers');
const NODE_URL = "CHAINSTACK_NODE_URL";
const provider = new ethers.JsonRpcProvider(NODE_URL);

const USDC = "0x3600000000000000000000000000000000000000";

const getCode = async () => {
    const code = await provider.getCode(USDC);
    console.log(`Is contract: ${code !== '0x'}`);
  };

getCode();
```

```python web3.py
from web3 import Web3

node_url = "CHAINSTACK_NODE_URL"
web3 = Web3(Web3.HTTPProvider(node_url))

USDC = "0x3600000000000000000000000000000000000000"
code = web3.eth.get_code(USDC)
print(f"Is contract: {code != b''}")
```

```bash cURL
curl -X POST "CHAINSTACK_NODE_URL" \
  -H "Content-Type: application/json" \
  -d '{"jsonrpc": "2.0", "method": "eth_getCode", "params": ["0x3600000000000000000000000000000000000000", "latest"], "id": 1}'
```
</CodeGroup>
