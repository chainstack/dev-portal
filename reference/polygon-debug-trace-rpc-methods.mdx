...
This tracer returns the call trace as a flat list in the Parity/OpenEthereum format, matching the output of the legacy `trace_*` methods. It is the closest drop-in replacement when migrating `trace_transaction`, `trace_block`, or `trace_call` to their `debug_*` equivalents.
