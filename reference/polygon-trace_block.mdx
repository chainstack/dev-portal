...
description: "Polygon API method that returns traces for all the transactions within a specific block. Available on Polygon via Chainstack JSON-RPC nodes."
---
...
<Warning>
**Not available on Polygon.** Chainstack serves Polygon on the Bor client (archive uses Bor with PBSS), which does not implement the Erigon-only `trace_*` namespace. Use [`debug_traceBlockByNumber`](/reference/polygon-traceblockbynumber) with the `flatCallTracer`, which returns equivalent per-transaction traces for a block on Bor in the same Parity flat format as `trace_block`.
</Warning>
