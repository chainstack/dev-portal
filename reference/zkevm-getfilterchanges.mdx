...
The `eth_getFilterChanges` method can poll a filter to periodically retrieve new data. You can, for example, create a blocks filter using [eth\_newBlockFilter](/reference/polygon_zkevm_node_api/eth_newBlockFilter) and periodically poll it to get new blocks.
