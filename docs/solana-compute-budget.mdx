...
* On [transaction v1](/docs/solana-transaction-v1), these settings move out of Compute Budget Program instructions and into a `transactionConfig` field on the message.
...
## Compute budget on transaction v1

Everything above describes legacy and v0 transactions. A [v1 transaction](/docs/solana-transaction-v1) carries the same settings in a `transactionConfig` field on the message rather than as Compute Budget Program instructions:
...
Two differences matter when you read v1 traffic:

* The priority fee is a total in lamports, not a rate in micro-lamports per compute unit. A v1 transaction pays `5000 × numRequiredSignatures + priorityFee`, with no further arithmetic.
* Code that derives limits or fees by scanning for Compute Budget instructions finds nothing on a v1 transaction. Read `transactionConfig` when it is present, and fall back to the instruction scan for legacy and v0.

To compare a v0 rate against a v1 total, apply the same formula as above: `SetComputeUnitPrice × SetComputeUnitLimit / 1,000,000`.
...
* [Solana: transaction v1 explained](/docs/solana-transaction-v1) — the `transactionConfig` field and the fee unit change
