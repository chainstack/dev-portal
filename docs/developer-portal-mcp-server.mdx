...
This will automatically configure the Chainstack Developer Portal MCP server to work with selected AI IDE by updating the `mcp.json` file. You do not need to manually update it.

To enable in the Amazon's Kiro IDE, just select any of the options, copy the printed output and paste it in your `~/.kiro/settings/mcp.json` file. Example:
...
```json
{
  "mcpServers": {
    ...
    "chainstack": {
      "command": "node",
      "args": [
        "/Users/ake/.mcp/chainstack/src/index.js"
      ]
    }
  }
}
