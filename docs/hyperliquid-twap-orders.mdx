...
**`activated`** — TWAP accepted by exchange, child order execution begins. You'll see this once when the TWAP starts.

**`completed`** — All size executed successfully. The TWAP ran to completion without issues.
...
**Scenario**: You hold a 10 BTC long position. You place a reduce-only TWAP sell for 15 BTC. The TWAP stops after selling 10 BTC—it won't open a 5 BTC short position. Without `"r": True`, you'd flip from 10 BTC long to 5 BTC short.
...
<Info>
Why include size in the combination? A leader might place two identical 30-minute BTC TWAPs with different sizes (e.g., 0.5 BTC and 1.0 BTC). Without size, they'd have the same combination key and you'd only mirror one.
</Info>
...
The key difference: Leader combination uses leader's size, follower combination uses follower's (potentially different) size. When a WebSocket event arrives, you check both:
...
When a leader cancels or terminates a TWAP, you need to cancel the corresponding follower TWAP. Here's where the asymmetry matters:
...
<Info>
If the leader places the same TWAP combination later (same asset, same parameters, same size), you want to mirror it again. Removing it from `leader_twap_combinations` allows re-processing.
</Info>
...
<AccordionGroup>
  <Accordion title="TWAP splits large orders over time">
    Reduces market impact from 5–10% slippage to 0.5–1% for illiquid assets.
  </Accordion>
