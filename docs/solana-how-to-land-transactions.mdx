...
For competitive or atomic flows — arbitrage, liquidations, high-contention mints — **Jito bundles** are a distinct delivery channel: an all-or-nothing group of up to five transactions that a Jito validator executes in order, prioritized by a tip you attach rather than by the priority fee. Most Solana stake runs the Jito client, so tips are a primary path for latency-sensitive transactions — complementary to SwQoS, not a replacement.

For live, continuously-measured landing rates comparing Warp against other providers, see the [Chainstack Compare dashboard](https://compare.chainstack.com/). To defend against sandwich attacks on the way in — including Jito's `dontfront` — see [Solana MEV protection](/docs/solana-mev-protection).
...
- For an aggressive path under congestion, send with `maxRetries: 0` and `skipPreflight: true`, then run your own loop: re-send the same signed transaction every few seconds and poll `getSignatureStatuses`, stopping when it confirms or the current block height passes `lastValidBlockHeight`.
