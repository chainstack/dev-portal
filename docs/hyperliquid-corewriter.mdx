...
HyperEVM uses a [dual-block architecture](/docs/hyperliquid-development): fast 1-second small blocks with a 3M gas limit, and slow 1-minute big blocks with a 30M gas limit. A small contract deploys fine in small blocks, but larger contracts need big blocks. To route your transactions to big blocks, send the HyperCore action `{"type": "evmUserModify", "usingBigBlocks": true}` from the deployer account, then unset it afterward.
