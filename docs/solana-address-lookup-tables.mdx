...
* Lookup tables are a v0 feature. [Transaction v1](/docs/solana-transaction-v1) does not support them and raises the size limit to 4096 bytes instead.

## Why you need address lookup tables

<Note>
Everything on this page applies to v0 transactions, which are not deprecated. A [v1 transaction](/docs/solana-transaction-v1) cannot reference a lookup table at all—it carries up to 64 addresses inline and gets a 4096-byte envelope to fit them in.
</Note>

Every v0 or legacy Solana transaction has a hard 1232-byte size limit. Each account address takes 32 bytes, so a transaction with ~30 accounts already exhausts most of the budget before adding instructions. You will hit this limit when:
...
On [transaction v1](/docs/solana-transaction-v1) the same scenarios fit inline: the 4096-byte envelope holds the full 64-address maximum without a table.
...
* [Solana: transaction v1 explained](/docs/solana-transaction-v1) — the format that replaces the need for lookup tables
