...
<Info>
  ...
  * **gRPC** — high-performance binary protocol for efficient data access (see [available services](#available-services))
  ...
  TRON nodes are full nodes with the complete block and transaction history from the genesis block, plus historical TRX balance lookups via `/wallet/getaccountbalance` and `/wallet/getblockbalance` (with a `block_identifier`). TRON has no archive-node concept — historical contract/account *state* cannot be queried at a past block. State queries such as `eth_getBalance`, `eth_getStorageAt`, and `eth_call` only accept the `latest` tag (and `triggerconstantcontract` runs against current state); this is a java-tron limitation, not a data-retention one.
</Info>
