...
If the user is migrating from another RPC provider — most commonly **QuickNode** or **Alchemy**, but also Infura, Ankr, or others — estimate their Chainstack cost and the savings versus their current bill.

5. Get the user's current usage from the source provider:
   - **QuickNode** — the Admin API exposes usage directly. Ask the user for a QuickNode API key (created at [dashboard.quicknode.com/api-keys](https://dashboard.quicknode.com/api-keys)) and fetch their billing-period usage:
     ...
     ```
     This returns API Credits used and remaining for the period. Per-method and per-chain breakdowns are at `/v0/usage/rpc/by-method` and `/v0/usage/rpc/by-chain`.
   - **Alchemy** — Alchemy has no usage API. Ask the user to read their Compute Units used and current plan from the dashboard (Settings → Usage and Settings → Billing).
   - **Other provider / no programmatic access** — ask the user for their approximate monthly request volume and current monthly spend.
