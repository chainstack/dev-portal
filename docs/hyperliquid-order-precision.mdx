---
title: "Handling order precision and market metadata for multi-asset trading"
...
**Lot size (order quantity)**: Never hardcode precision. Query `szDecimals` from metadata for each asset to determine valid order size increments.

**Tick size (price)**: Prices must have ≤5 significant figures with max 6 decimals (perps) or 8 decimals (spot). Remove trailing zeros when signing.

**Handle all asset formats**: Support `@index` for spot, `PAIR/USDC` notation, and direct `SYMBOL` for perpetuals.

**Validate before placing**: Check minimum notional (~\$10 USDC) and precision requirements to avoid rejections.

**Use proper rounding**: Python's `round()` with dynamic decimal places from `szDecimals` field in metadata.

**Cache metadata**: Reduce API calls for high-frequency trading—metadata doesn't change frequently, but refresh periodically.
