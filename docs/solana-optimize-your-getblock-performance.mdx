...
* `maxSupportedTransactionVersion` — the highest transaction version your client can parse. Set it to `1` for [version 1 transactions](https://solana.com/upgrades/larger-transaction-sizes), which Solana mainnet now produces. A lower value, or an omitted field, makes the call respond with error `-32015` when the block holds a newer transaction.
...
<CodeGroup>
  ...
  curl -X POST CHAINSTACK_SOLANA_URL \
    ...
    -d '{
      ...
      "id": 1,
      "method": "getBlock",
      "params": [329849011, {"encoding": "json", "maxSupportedTransactionVersion": 1}]
    }' \
...
<CodeGroup>
  ...
  class BlockCache:
      ...
      def get_block(self, slot: int, encoding: str = "json", tx_details: str = "full",
                    max_supported_transaction_version: Optional[int] = 1) -> Optional[Dict[str, Any]]:
          """
