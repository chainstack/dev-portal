...
2. Python 3.11+ with the dependencies installed through [uv](https://github.com/astral-sh/uv):
   <CodeGroup>
     ```shell Shell
     uv sync
     ```
   </CodeGroup>
3. A `.env` file in the repository root that includes your WebSocket endpoint as `SOLANA_NODE_WSS_ENDPOINT`.
...
The repository's `cookbook/pumpfun/listen/` directory has a listener for each method — `pumpfun_listen_tokens_logsubscribe.py` (logs only), `pumpfun_listen_tokens_blocksubscribe.py`, `pumpfun_listen_tokens_geyser.py`, and `pumpfun_listen_tokens_pumpportal.py` — and [`tools/compare_listeners.py`](https://github.com/chainstacklabs/pumpfun-bonkfun-bot/blob/main/tools/compare_listeners.py) runs them side by side and reports which detects a new token first:
