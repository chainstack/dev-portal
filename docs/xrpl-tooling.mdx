...
XRP Ledger Global Nodes are [Extended](/docs/extended-nodes) — they keep a **rolling window of recent ledgers** larger than a default `rippled` server holds, but not the full chain. Read `complete_ledgers` from `server_info` at runtime and work within it — the lower bound advances as the node prunes, so treat any specific depth as a moving target rather than a guarantee.
