...
* The cookbook provides standalone reference scripts on Anchor discriminators, price fetching, and subscribing to Solana websockets via `logsSubscribe`, `blockSubscribe`, and Geyser.
...
See the full project code, including the cookbook of example scripts, in the respective [GitHub repository](https://github.com/chainstacklabs/pumpfun-bonkfun-bot).

## Overview

The project uses the pump.fun IDL to interact with the program and the accounts.

It's very useful to know that the pump.fun program and all the associated accounts is built with the [Anchor framework](https://www.anchor-lang.com/), as a lot of the decoding comes back to Anchor, like the [8 byte instruction signatures/discriminators](https://www.anchor-lang.com/docs/basics/idl).
...
* The [Anchor instruction discriminators](https://www.anchor-lang.com/docs/basics/idl) are precalculated. See [`cookbook/solana/anchor_calculate_discriminator.py`](https://github.com/chainstacklabs/pumpfun-bonkfun-bot/blob/main/cookbook/solana/anchor_calculate_discriminator.py).
...
The [`cookbook/`](https://github.com/chainstacklabs/pumpfun-bonkfun-bot/tree/main/cookbook) directory has standalone scripts for understanding how the bot works and how to interact with Solana, grouped by platform and then by action. Each script takes its input as a command-line argument—run it with `uv run <path>`, and pass `--help` to see the options. Highlights:

* [`cookbook/pumpfun/listen/pumpfun_listen_tokens_logsubscribe.py`](https://github.com/chainstacklabs/pumpfun-bonkfun-bot/blob/main/cookbook/pumpfun/listen/pumpfun_listen_tokens_logsubscribe.py) — detects new tokens using only `logsSubscribe`, deriving the associated bonding curve on the fly (no extra `getTransaction` call)
* [`tools/compare_listeners.py`](https://github.com/chainstacklabs/pumpfun-bonkfun-bot/blob/main/tools/compare_listeners.py) — runs the listener methods side by side to see which detects new tokens first
* [`cookbook/pumpfun/read/pumpfun_read_curve.py`](https://github.com/chainstacklabs/pumpfun-bonkfun-bot/blob/main/cookbook/pumpfun/read/pumpfun_read_curve.py) — checks whether a token's bonding curve is still active or has completed
* [`cookbook/pumpfun/graduation/pumpfun_listen_migrations_logsubscribe.py`](https://github.com/chainstacklabs/pumpfun-bonkfun-bot/blob/main/cookbook/pumpfun/graduation/pumpfun_listen_migrations_logsubscribe.py) — detects graduations to PumpSwap; see [Listening to pump.fun migrations to PumpSwap](/docs/solana-listening-to-pumpfun-migrations-to-raydium)
* [`cookbook/pumpfun/read/pumpfun_derive_curve_addresses.py`](https://github.com/chainstacklabs/pumpfun-bonkfun-bot/blob/main/cookbook/pumpfun/read/pumpfun_derive_curve_addresses.py) — derives the bonding curve and associated bonding curve addresses for a token
* [`cookbook/solana/anchor_calculate_discriminator.py`](https://github.com/chainstacklabs/pumpfun-bonkfun-bot/blob/main/cookbook/solana/anchor_calculate_discriminator.py) — calculates the 8-byte Anchor discriminator for any instruction or account name
* [`cookbook/pumpswap/`](https://github.com/chainstacklabs/pumpfun-bonkfun-bot/tree/main/cookbook/pumpswap) — buy and sell examples against the PumpSwap AMM
* [`cookbook/pumpfun/trade/pumpfun_buy_token_v2.py`](https://github.com/chainstacklabs/pumpfun-bonkfun-bot/blob/main/cookbook/pumpfun/trade/pumpfun_buy_token_v2.py) / [`pumpfun_sell_token_v2.py`](https://github.com/chainstacklabs/pumpfun-bonkfun-bot/blob/main/cookbook/pumpfun/trade/pumpfun_sell_token_v2.py) — buy or sell a token you name by its mint address; the buy script supports `--dry-run` to simulate instead of spending
* [`cookbook/pumpfun/trade/pumpfun_snipe_token_blocksubscribe.py`](https://github.com/chainstacklabs/pumpfun-bonkfun-bot/blob/main/cookbook/pumpfun/trade/pumpfun_snipe_token_blocksubscribe.py) — waits for the next token created on pump.fun over `blockSubscribe`, then buys it; this is a sniper, not a manual buy of a specific token

<Warning>
The trade scripts submit real transactions and spend real funds. Read the module docstring before running one.
</Warning>
