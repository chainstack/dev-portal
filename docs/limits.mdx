...
[`getProgramAccounts`](/reference/solana-getprogramaccounts) is a heavy, rate-limited method — up to 20 RPS with filters. Throughput depends on how many accounts your query returns, so always narrow it with `dataSize` or `memcmp` filters. Unfiltered requests to large programs are blocked.
