---
title: "Ponder: Building blockchain application backends with Chainstack"
---
...
* **Ethereum Mainnet** - for established DeFi protocols
  ...
  <Step>
   ...
   <CodeGroup>
     ...
     ponder.on("UsdcMainnet:Transfer", async ({ event, context }) => {
       ...
       if (from !== zeroAddress) {
         await updateBalance(context, from, event.log.address, "ethereum", -value);
       }
       
       if (to !== zeroAddress) {
         await updateBalance(context, to, event.log.address, "ethereum", value);
       }

       // Update token statistics
       await updateTokenStats(context, event.log.address, "ethereum", value);
     });
     ...
     ponder.on("UsdcPolygon:Transfer", async ({ event, context }) => {
       ...
       if (from !== zeroAddress) {
         await updateBalance(context, from, event.log.address, "polygon", -value);
       }
       
       if (to !== zeroAddress) {
         await updateBalance(context, to, event.log.address, "polygon", value);
       }

       await updateTokenStats(context, event.log.address, "polygon", value);
     });
     ...
     ponder.on("UsdcBase:Transfer", async ({ event, context }) => {
       ...
       if (from !== zeroAddress) {
         await updateBalance(context, from, event.log.address, "base", -value);
       }
       
       if (to !== zeroAddress) {
         await updateBalance(context, to, event.log.address, "base", value);
       }

       await updateTokenStats(context, event.log.address, "base", value);
     });
     ...
     async function updateBalance(
       ...
       if (existingBalance) {
         ...
         const newBalance = existingBalance.balance + change;
   ...
   - Calculates aggregate statistics per token contract
  </Step>
</Steps>
...
<Steps>
  <Step>
   ...
   <CodeGroup>
     ...
     09:14:25.445 INFO  indexing Started indexing from blocks mainnet: 18000000, polygon: 45000000, base: 2000000
