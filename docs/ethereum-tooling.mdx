...
<Steps>
  ...
  <Step title="Initialize HTTP or WebSocket provider">
   <CodeGroup>
...
<Steps>
  ...
  <Step title="Configure environment in Foundry">
  ...
  <Step title="Before deploying, ensure that your contracts are compiled. This step is crucial to avoid deployment issues.">
   <CodeGroup>
     ```bash Bash
     forge build
     ```
  ...
  <Step title="To use the Chainstack endpoint for deployment, you can use the following command:">
   <CodeGroup>
     ```bash Bash
  ...
  <Step title="Interact with the blockchain using Cast. Use the following command to query the balance of an Ethereum address:">
   <CodeGroup>
     ```bash Bash
...
<Steps>
  <Step title="Install Hardhat and other dependencies">
   <CodeGroup>
     ```Text Bash
     npm init -y
  ...
  <Step title="Configure environment in Hardhat">
   Create a `.env` file in your project root and add your Chainstack endpoint and private key:
  ...
  <Step title="To deploy your contracts using the Chainstack endpoint, you can create a deployment script in the `scripts` folder, for example deploy.js:">
   <CodeGroup>
     ```javascript Javascript
  ...
  <Step title="Use the appropriate network flag (mainnet or devnet) to deploy to the respective network">
   <CodeGroup>
     ```bash Bash
...
<Steps>
  <Step title="Clone the Scaffold-ETH 2 repository">
   <CodeGroup>
     ```bash Bash
  ...
  <Step title="Create a  .env.local  file in the root directory of your project">
   <CodeGroup>
     ```bash Bash
     touch .env.local
     ```
  ...
  <Step title="Add your Chainstack endpoint and private key to the .env.local file">
   ```
   CHAINSTACK_MAINNET_ENDPOINT=https://your-chainstack-mainnet-url
   CHAINSTACK_DEVNET_ENDPOINT=https://your-chainstack-devnet-url
  ...
  <Step title="Modify the packages/hardhat/hardhat.config.ts file to configure the Chainstack endpoint. Locate the networks section and add or modify the configuration for the Chainstack networks">
   <CodeGroup>
     ```javascript Bash
  ...
  <Step title="Compile contracts using Hardhat">
   <CodeGroup>
     ```bash Bash
     cd packages/hardhat
  ...
  <Step title="To deploy your contracts using the Chainstack endpoint, run">
   <CodeGroup>
     ```Text Bash
  ...
  <Step title="To start the NextJS app with the Chainstack configuration">
   <CodeGroup>
     ```Text Bash
     yarn start
     ```
...
<Steps>
  <Step title="Install Truffle and other dependencies">
   <CodeGroup>
     ```Text Bash
  ...
  <Step title="Configure environment in Truffle">
  ...
  <Step title="Create a migration script in the migrations folder to deploy your contract (migrations/1_deploy_contract_name.js)">
   <CodeGroup>
     ```javascript Javascript
  ...
  <Step title="To deploy your contract using the Chainstack endpoint, run">
   <CodeGroup>
     ```bash Bash
...
<Steps>
  <Step title="Install Ape and dependencies">
   <CodeGroup>
     ```Text Bash
     pip install eth-ape
     ```
  ...
  <Step title="Configure environment in Ape">
  ...
  <Step title="Import an account. You’ll be prompted to enter a private key and pass phrase to encrypt it.">
   <CodeGroup>
     ```bash Bash
  ...
  <Step title="To deploy your contracts using the Chainstack endpoint, you can create a deployment script, for example scripts/deploy.py">
   <CodeGroup>
     ```python Python
  ...
  <Step title="Compile contracts. Before deploying, ensure your contracts are compiled. Ape automatically compiles contracts when deploying, but you can manually compile them if needed">
   <CodeGroup>
     ```bash Bash
     ape compile
     ```
  ...
  <Step title="Run the deployment script using">
   <CodeGroup>
     ```Text Bash
...
Configure [Brownie](https://www.google.com/url?sa=t\&rct=j\&q=\&esrc=s\&source=web\&cd=\&ved=2ahUKEwjYhp_e7KuHAxX4VfEDHSpMChEQFnoECBgQAQ\&url=https%3A%2F%2Fgithub.com%2Feth-brownie%2Fbrownie\&usg=AOvVaw0edmb1wsW78Y-AhUEmQwpz\&opi=89978449) to deploy contracts using Chainstack Ethereum nodes.

<Steps>
  <Step title="Create a Python virtual environment and install Brownie">
   ```bash
   python -m venv venv
  ...
  <Step title="Configure environment in Brownie">
