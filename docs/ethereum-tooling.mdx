...
To add a custom RPC to Rabby, open the wallet and go to **Settings** > **Modify RPC URL** to replace the node Rabby uses for a chain it already supports. See [Adding a custom RPC endpoint to Rabby](/docs/adding-a-custom-rpc-endpoint-to-rabby).
