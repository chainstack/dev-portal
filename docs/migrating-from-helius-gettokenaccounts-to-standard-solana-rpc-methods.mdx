...
Here's a *sample* of how you'd be using the standard [getProgramAccounts](/reference/solana_node_api/getProgramAccounts) method to monitor the holders of a token and then dumping the data to a PostgreSQL DB and keeping key-value store of accounts holding the token and the number of tokens up to date.
