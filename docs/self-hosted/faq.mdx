...
Yes — Ethereum (all networks), Optimism Mainnet, and Robinhood Chain Mainnet presets ship with snapshot bootstrap pre-configured. Those nodes deploy from a pre-built chain snapshot rather than syncing from genesis, reducing initial sync from days to hours. Other deployments sync without a snapshot.
