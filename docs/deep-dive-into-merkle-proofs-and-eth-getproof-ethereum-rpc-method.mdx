...
* `ARRAY` — the array of 32-byte storage keys that need to be proven and included. See [eth\_getStorageAt](/api-reference/ethereum_node_api/accounts_info/eth_getStorageAt) in the API reference.
...
* `balance`: `QUANTITY` — the balance of the account. See [eth\_getBalance](/api-reference/ethereum_node_api/accounts_info/eth_getBalance) in the API reference.
...
* `nonce`: `QUANTITY` — the nonce of the account. See [eth\_getTransactionCount](/api-reference/ethereum_node_api/accounts_info/eth_getTransactionCount).
