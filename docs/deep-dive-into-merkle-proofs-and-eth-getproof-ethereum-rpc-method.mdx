...
* `ARRAY` — the array of 32-byte storage keys that need to be proven and included. See [eth\_getStorageAt](/reference/ethereum_node_api/accounts_info/eth_getStorageAt) in the API reference.
...
* `balance`: `QUANTITY` — the balance of the account. See [eth\_getBalance](/reference/ethereum_node_api/accounts_info/eth_getBalance) in the API reference.
...
* `nonce`: `QUANTITY` — the nonce of the account. See [eth\_getTransactionCount](/reference/ethereum_node_api/accounts_info/eth_getTransactionCount).
