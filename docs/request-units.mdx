...
| TRON | No archive split — every request is billed as full. The nodes themselves run in archive mode and serve the complete block and transaction history; historical state queries don't exist on TRON. See [Historical data availability](/docs/tron-tooling#historical-data-availability). |
| Bitcoin, Sui, Aptos, Starknet, Polkadot, opBNB, Harmony | No archive split — every request is billed as full |
