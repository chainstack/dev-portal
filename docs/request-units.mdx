...
<Note>
  Archive in this table is a request billing class — it is not the same as the archive node mode in the [networks catalog](/docs/protocols-networks), which describes what data a node retains. A protocol can run archive-mode nodes and still bill every request as full. TRON is one example: the nodes serve the complete block and transaction history, yet there is no archive billing class — see [TRON historical data availability](/docs/tron-tooling#historical-data-availability).
</Note>
