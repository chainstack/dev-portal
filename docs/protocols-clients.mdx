...
[AvalancheGo](https://github.com/ava-labs/avalanchego) is the node implementation for the Avalanche network in Golang. You can find API documentation in the [Chainstack Avalanche API reference](/api-reference/avalanche-getting-started).
...
[zkEVM Node](https://github.com/0xPolygonHermez/zkevm-node) is a Go implementation of a node that operates the Polygon zkEVM network. For available methods, see [Chainstack zkEVM API reference](/api-reference/zkevm-getting-started) and the [official zkEVM documentation](https://developer.arbitrum.io/).
...
[Aurora](https://github.com/aurora-is-near/aurora-engine) operates on top of NEAR. It is EVM-compatible, and all its available JSON-RPC methods can be found in the official [Aurora documentation](https://doc.aurora.dev/evm/rpc/) and [GitHub repository](https://github.com/aurora-is-near/relayer2-public).
...
[Bitcoin Core](https://bitcoin.org/en/download) connects to the Bitcoin peer-to-peer network to download and validate blocks and transactions. See available JSON-RPC API methods in the [Bitcoin Postman collection](/api-reference/bitcoin-rpc-methods-postman-collection).
