...
The execution client is `base-reth-node` (built on [Reth](https://github.com/paradigmxyz/reth)) and the consensus client is `base-consensus`. Base introduced this native stack with the Azul upgrade (mainnet May 28, 2026), replacing op-geth and op-node.
