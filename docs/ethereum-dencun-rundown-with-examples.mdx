...
Practically, running an [eth\_getBlockByNumber | Ethereum](/api-reference/ethereum_node_api/blocks_info/eth_getBlockByNumber) after Dencun adds `parentBeaconBlockRoot` in the block details:
...
Now let's do an [eth\_getBlockByNumber | Ethereum](/api-reference/ethereum_node_api/blocks_info/eth_getBlockByNumber) on block `5475939` to get the block's `parentBeaconBlockRoot` as discussed previously:
...
Use the slot number in a [Retrieve blob sidecar](/api-reference/ethereum_beacon_chain_api/state/getBlobSidecarByRoot) call to the consensus layer:
