...
| `-32012` | Scan aborted | A Solana account-scan method (`getProgramAccounts`, `getTokenAccountsByOwner`, `getTokenAccountsByDelegate`, `getSupply`) exceeded its execution-time or memory limit and was aborted instead of hanging. Narrow the query — for example with `dataSize` or `memcmp` filters on `getProgramAccounts`. See [Solana scan method limits](/docs/limits#solana-scan-method-limits). |
