...
Our example transaction is included in [block 6090748](https://sepolia.etherscan.io/txs?block=6090748\&isblob=true) on the execution layer. The call we are going to use is [Retrieve blob sidecar](/api-reference/ethereum_beacon_chain_api/state/getBlobSidecarByRoot).
...
Remember that the blob versioned hash is stored on the execution layer. To retrieve it, you can do a simple [eth\_getTransactionByHash](/api-reference/ethereum_node_api/transaction_info/eth_getTransactionByHash).
