...
Replace `TOKEN_ADDRESS` with the Solana address of the token you want to check. The script derives the associated bonding curve address from the token address that you provide and then makes a [getAccountInfo | Solana](/reference/solana_node_api/getAccountInfo) call to the bonding curve.
