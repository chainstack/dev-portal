...
* Use `pumpfun_read_curve.py` to check whether a token's curve is still active or completed.
* Use `pumpfun_listen_migrations_logsubscribe.py` (logsSubscribe on the migration program) or `pumpfun_listen_migrations_programsubscribe.py` (programSubscribe on the PumpSwap program) to track live graduations.

<Warning>
This guide previously covered migration to **Raydium**. On **March 20, 2025** pump.fun launched its own AMM, [PumpSwap](https://x.com/pumpdotfun/status/1902762309950292010), and graduated tokens now migrate to PumpSwap instead of Raydium. The bonding-curve mechanics and the migration program are the same; only the destination AMM changed. The old Raydium listener is no longer in the repository.
</Warning>
...
This article is an add-on for these scripts in the [pumpfun-bonkfun-bot repository](https://github.com/chainstacklabs/pumpfun-bonkfun-bot):

<CardGroup>
  <Card title="pumpfun_read_curve.py" href="https://github.com/chainstacklabs/pumpfun-bonkfun-bot/blob/main/cookbook/pumpfun/read/pumpfun_read_curve.py" icon="angle-right" horizontal />
  <Card title="pumpfun_listen_migrations_logsubscribe.py" href="https://github.com/chainstacklabs/pumpfun-bonkfun-bot/blob/main/cookbook/pumpfun/graduation/pumpfun_listen_migrations_logsubscribe.py" icon="angle-right" horizontal />
  <Card title="pumpfun_listen_migrations_programsubscribe.py" href="https://github.com/chainstacklabs/pumpfun-bonkfun-bot/blob/main/cookbook/pumpfun/graduation/pumpfun_listen_migrations_programsubscribe.py" icon="angle-right" horizontal />
  <Card title="compare_migration_listeners.py" href="https://github.com/chainstacklabs/pumpfun-bonkfun-bot/blob/main/tools/compare_migration_listeners.py" icon="angle-right" horizontal />
</CardGroup>
...
* Clone the [pumpfun-bonkfun-bot GitHub repository](https://github.com/chainstacklabs/pumpfun-bonkfun-bot)
* Install the dependencies with [uv](https://github.com/astral-sh/uv): `uv sync`
* Provide the node HTTP and WebSocket endpoints in your `.env` file as `SOLANA_NODE_RPC_ENDPOINT` and `SOLANA_NODE_WSS_ENDPOINT`
...
1. `cookbook/pumpfun/read/pumpfun_read_curve.py` — checks whether a token is still on the bonding curve or has completed and graduated
2. `cookbook/pumpfun/graduation/pumpfun_listen_migrations_logsubscribe.py` — monitors real-time graduations by listening to the migration program's events
3. `cookbook/pumpfun/graduation/pumpfun_listen_migrations_programsubscribe.py` — monitors real-time graduations by watching for new PumpSwap pool accounts
4. `tools/compare_migration_listeners.py` — runs both listeners side by side to compare detection latency

### Checking bonding curve status

The `pumpfun_read_curve.py` script lets you check whether a token's bonding curve is still active or has completed and graduated. It derives the bonding curve address from the token mint and makes a [getAccountInfo | Solana](/reference/solana-getaccountinfo) call to read the curve state from the pump.fun program (`6EF8rrecthR5Dkzon8Nwu78hRvfCKubJ14M5uBEwF6P`).

<Note>
The bonding curve account layout has evolved. The current state includes a `creator` field (and an `is_mayhem_mode` flag), so make sure your decoder matches the latest account struct in the repository's `pumpfun_read_curve.py`. The `complete` flag itself is unchanged.
</Note>
...
<CodeGroup>
  ```shell Shell
  uv run cookbook/pumpfun/read/pumpfun_read_curve.py TOKEN_ADDRESS
  ```
</CodeGroup>
...
**Option 2 — watch for new PumpSwap pools (programSubscribe).** `pumpfun_listen_migrations_programsubscribe.py` subscribes to the PumpSwap program `pAMMBay6oceH9fJKBRHGP5D4bD4sWpmSwMn52FMfXEA` and detects new pool (market) accounts as they are created, filtering by the pool account discriminator and length. This catches every new pool, but consumes a high volume of WebSocket messages.

Run `tools/compare_migration_listeners.py` to run both at once and measure which detects a given graduation first across your RPC providers.
