...
Stellar Global Nodes are [Extended](/docs/extended-nodes) — they keep an extended rolling window of recent ledgers rather than history from genesis. Billing is independent of the window — every Stellar request is billed as full, at 1 RU; see [Request units](/docs/request-units).
