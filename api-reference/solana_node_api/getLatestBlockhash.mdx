...
For our example, let's do a [getBlock](/api-reference/solana_node_api/getBlocks) call, the naming of which confuses things even further as you actually need to provide the slot number as a value in the `getBlock` call:
...
You can get the current block (not slot!) height with the [getBlockHeight](/api-reference/solana_node_api/getBlockHeight) call.

And you can get the current slot height with the [getSlot](/api-reference/solana_node_api/getSlot) call.
...
* Slots and blocks have a mapping. The incorrectly named method [getBlock](/api-reference/solana_node_api/getBlocks) actually fetches you a slot. And the fetched slot contains the actual block number in that slot and all the block data like hash, time, transactions etc.
* You can get the latest slot number (slot height) with the [getSlot](/api-reference/solana_node_api/getSlot) call.
* You can get the latest block number (block height) with the [getBlockHeight](/api-reference/solana_node_api/getBlockHeight) call.
