...
* [eth\_newFilter](/api-reference/ethereum_node_api/logs_and_events/eth_newFilter)
* [eth\_newPendingTransactionFilter](/api-reference/ethereum_node_api/transaction_info/eth_newPendingTransactionFilter)
* [eth\_newBlockFilter](/api-reference/ethereum_node_api/blocks_info/eth_newBlockFilter)
...
One use case for `eth_uninstallFilter` is to optimize resource usage in a DApp. When a DApp needs to monitor events on the blockchain, it can create a filter using [eth\_newFilter](/api-reference/ethereum_node_api/logs_and_events/eth_newFilter),[eth\_newPendingTransactionFilter](/api-reference/ethereum_node_api/transaction_info/eth_newPendingTransactionFilter), or [eth\_newBlockFilter](/api-reference/ethereum_node_api/blocks_info/eth_newBlockFilter) to listen for specific events or blocks.
...
Inside the `getNewBlocks` function, the [eth\_newBlockFilter](/api-reference/ethereum_node_api/blocks_info/eth_newBlockFilter) JSON-RPC method is called to create a new filter to watch for new blocks. The filter ID is returned and stored in the `filterId` variable. An array stores the blocks fetched.

Inside the loop, the [eth\_getFilterChanges](/api-reference/ethereum_node_api/filter_handling/eth_getFilterChanges) JSON-RPC method is called to retrieve any new blocks that have arrived since the filter was created. If there are no new blocks, the code waits a certain amount of time before trying again. If there are new blocks, they are stored in the `blocks` array.
