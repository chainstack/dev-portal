...
One use case for `eth_uninstallFilter` is to optimize resource usage in a DApp. When a DApp needs to monitor events on the blockchain, it can create a filter using [eth\_newFilter](/api-reference/aurora_node_api/eth_newFilter) or [eth\_newBlockFilter](/api-reference/aurora_node_api/eth_newBlockFilter) to listen for specific events or blocks.
