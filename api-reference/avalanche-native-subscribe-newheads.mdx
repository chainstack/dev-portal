...
* `object`— a block object with the following fields:
  ...
  * `transactions` — an array of transaction objects. See [eth\_getTransactionByHash](/api-reference/avalanche_node_api/transactions_info/eth_getTransactionByHash) for the exact shape.
...
Use [eth\_unsubscribe](/api-reference/avalanche-native-unsubscribe) to remove the subscription.
