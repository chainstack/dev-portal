...
* `array` — the array of 32-byte storage keys that need to be proven and included. See [eth\_getStorageAt](/api-reference/cronos_node_api/eth_getStorageAt).
...
* `balance` — represents the account balance. Refer to [eth\_getBalance](/api-reference/cronos_node_api/eth_getBalance).
...
* `nonce` — refers to the account's nonce. See [eth\_getTransactionCount](/api-reference/cronos_node_api/eth_getTransactionCount).
