...
`boolean` — keep the [subscribe("syncing")](/api-reference/ethereum-subscribesyncing) subscription if `true`.
...
The following code is an example using the [subscribe("newBlockHeaders")](/api-reference/ethereum-subscribenewblockheaders) method, which will be stopped and removed after one minute using `clearSubscriptions`:
