...
web3.js subscription equivalent to [eth\_newFilter](/api-reference/arbitrum_node_api/logs_and_events/eth_newFilter). `subscribe("logs")` allows developers to subscribe to real-time updates about new event logs on the Arbitrum blockchain; the application will receive notifications whenever new logs matching the filter are emitted.
