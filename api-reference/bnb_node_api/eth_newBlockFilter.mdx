...
BNB API method that creates a filter that watches for new blocks on the blockchain. It returns a filter ID, which can be used to retrieve the results using the [eth\_getFilterChanges](/api-reference/bnb_node_api/eth_getFilterChanges) method. The `eth_newBlockFilter` method is useful for developers who must be notified of new blocks on the blockchain in real-time.
...
* [eth\_getFilterChanges](/api-reference/bnb_node_api/eth_getFilterChanges) to retrieve updates
