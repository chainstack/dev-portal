...
Ronin API method allowing developers to receive real-time notifications regarding new block headers on the Ronin blockchain; it sends notifications whenever a new block is added.
...
* `object`— a block object with the following fields:
  ...
  + `totaldifficulty` — `null`. This field is obsolete as it was used during Ethereum's Proof of Work (PoW) era. Ronin uses Proof of Authority (PoA) consensus and this value is not relevant.
