...
* `object`— a block object with the following fields:
  ...
  + `totaldifficulty` — `null`. This field is obsolete as it was used during Ethereum's Proof of Work (PoW) era. As an Ethereum L2, Optimism inherits Ethereum's consensus mechanism and this value is no longer relevant.
...
Use [eth\_unsubscribe | Optimism](/api-reference/optimism_node_api/eth_unsubscribe) to remove the subscription.
